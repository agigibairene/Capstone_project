...
export default function SideBar({ menuItems, activeItem, setActiveItem, full_name, initial, role }: Props) {
  ...
  function handleLogout() {
    dispatch(logout());
    dispatch(resetKYCState());  // ✅ clear KYC data on logout
    navigate('/login');
  }

  return (
    <aside className="h-screen bg-white/20 backdrop-blur-sm border-r border-white/30 flex flex-col fixed z-10 w-16 sm:w-64 transition-all duration-300 group">
