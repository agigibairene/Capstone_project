...
export default function InvestorDashboard() {
  ...
  const { kycData, loading, error } = useSelector(
        ...
        >
          ...
          <div className="flex flex-col flex-1 overflow-hidden pt-16 lg:pt-0">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex-1">
                    <p className="text-white text-sm md:text-lg font-medium">
                      Welcome back,{" "}
                      <span className="hidden sm:inline">{userKYC.full_name}</span>
                      <span className="sm:hidden">
                        {userKYC.full_name.split(" ")[0]}
                      </span>{" "}
                      👋
                    </p>
            ...
            <main className="flex-1 ml-20 lg:ml-2 no-scrollbar overflow-y-auto p-2 lg:p-4">
              <div className="max-w-full">{mainContent[activeItem]}</div>
            </main>
          </div>
        </div>
      </div>
